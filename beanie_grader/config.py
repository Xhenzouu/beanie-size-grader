...
from beanie_grader.beanie_config import PRODUCT_CONFIG  # Import default config for fallback
...
def load_config(path: str) -> dict:
    ...
    if not path or path.strip() == "":
        return PRODUCT_CONFIG  # Fallback if no path provided
    
    config_path = Path(path)
    
    if not config_path.exists():
        print(f"Warning: Config file not found: {path}. Using default config.")
        return PRODUCT_CONFIG  # Fallback instead of raising error
    ...
    try:
        ...
        return PRODUCT_CONFIG  # Fallback on any loading error